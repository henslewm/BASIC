...
TxOffset = TimeSerial(0,0,0)
TxInterval = TimeSerial(1,0,0)
TxWhileRecordingOff = False

Const MaxModemInitFail = 5          'Set max number of allowed failures it initialize modem 
...
Public Declare Sub ForceMTMCheck(RxMsg)
...
Sub START_RECORDING
   ...
   Do
	...
	If ComPortCounter > 30 Then
		Call ClosePort(IridiumHandle)
	End If
   Loop Until Not COMPortInUse					'XXXXXXX
   ...
   If IridiumHandle Then
      ...
      'The code below is only for Bhutan. NOAA modems get power from Com port
      'as of 06/18/2012
      'StatusMsg "Powering on iridium modem"
      'Power 1, 1
      ...
      'Shouldn't need to power off modem at this time.                                         'XXXXXX
      'StatusMsg "Powering off iridium modem - "&(IridiumHandle)
      'Power 1, 0
...
Public Function Iridium_Handler 
   ...
   'If iRecOn = -1 Then
	 ...
	 'The TxInterval is used with mod division to make sure transmissions of WL data happen
 	 'at the appropriate time.
         nOffset = tNow Mod TxInterval
         tNow = tNow - nOffset
         If ((tNow mod TxInterval) = 0) and (nOffset = TxOffset) Then
           StatusMsg "Time for normal transmission"
	   'The FormatIridiumData is the std GOES message for NOS as of  06/2012
           TxMsg = FormatIridiumData                                    
           'TxMsg = TxMsg + AddCRC(TxMsg)
		If TxWhileRecordingOff AND (SBDStarted = 0) Then
			StatusMsg "Recording is Off, we won't transmit"
		Else
			Call SendSBD(TxMsg, MsgInSBD, MsgBytes, NumMsgQueued, SendOK)
			StatusMsg "Recording is on so we transmit"
		End If
...
Public Sub SendSBD(TxMsg, MsgInSBD, MsgBytes, NumMsgQueued, SendOK)
  ...
  'SatLink2 
    ...
    iTry = 0
    Do
       Statusmsg "Iridium - AT+SBDIXA"
       iTry = iTry + 1
       StatusMsg "Iridium - Attempt #" & Format("%i",iTry) & " to send SBD Message"
       txt = HayesCommand(IridiumHandle, "AT+SBDIXA", 90)
...
Public Function ReceiveSBD(RxMsg)
  ...
  sResult = ReadPort(IridiumHandle, 2, BytesRead, BytesRemain, 100)              
...
'********************* FUNCTION Force MTM Mailbox Check ************************

Public Sub ForceMTMCheck(RxMsg)
	'Try to limit who's accessing this COM Port.
	COMPortInUse = True

	'+SBDI:<MO status>,<MOMSN>,<MT status>,<MTMSN>,<MT length>,<MT queued>
	
	Cmd = "AT+SBDD2"
	txt = HayesCommand(IridiumHandle, Cmd, 20)
	StatusMsg "Response from Clearing Both Buffers: " &txt
	
	Cmd = "AT+SBDI"
	txt = HayesCommand(IridiumHandle, Cmd, 20)
	StatusMsg "Response from Gateway +SBDI: " &txt

   	Cmd = "AT+SBDRT"
	txt = HayesCommand(IridiumHandle, Cmd, 20)
	StatusMsg "Response from +SBDRT: " &txt
