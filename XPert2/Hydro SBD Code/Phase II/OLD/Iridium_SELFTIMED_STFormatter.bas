...
'     is done by writing the message to IRIDIUM.DAT.  That way, the user can define a
...
Public Function SELFTIMED_STFormatter
   'Note here that Selftimed_STFormatter is the current message built by the C code
   'StatusMsg "I am inside the custom formatter!"
   iDatFile = FreeFile
